...
- Ambos lados usan `kSecClassGenericPassword` en el Keychain de protección de datos (`kSecUseDataProtectionKeychain`), con `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly`: la identidad es por dispositivo y no migra con un respaldo. En macOS eso exige que el agente esté firmado con un equipo de desarrollo (`Configs/Local.xcconfig`); a cambio no hay diálogos del llavero de login al re-firmar en cada build. Si el Keychain falla, las apps siguen funcionando con lo que hay en memoria y muestran un error localizado (detalle técnico solo en Debug).
