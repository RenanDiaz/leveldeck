...
> Derived from the "v2" section of `INTENT.md` (approved). Status: **2.1**. Draft 2.0 listed ten open decisions in §21 with a recommended option each; all ten were decided (the recommended option in every case, with additions) and 2.1 incorporates them where they apply: §14.1, §16.2, §17.2, §19 (Phases 6 and 7), §20.6 (G13) and §21.
>
...
- Paired devices (with revocation), the login item toggle and "About" (version, protocol) move to a **Settings window** (`Settings` scene, `⌘,`, also reachable from the menu), per decision D7. The menu keeps what's used daily: the strips, service status, "Pair New Device…", "Settings…" and Quit. Because the agent is `LSUIElement`, opening the window from the menu has to activate the app (`NSApplication.activate`, as the pairing window already does) so it comes to the front instead of opening behind the current app.
- While a slider in the menu is being dragged, the displayed list of strips is frozen (same rule as the client, §16.2): a device plugged in during the drag gets its strip when the drag ends.
...
- **Identity by `StripID`, never by position.** Drags, throttles, echo gates and invalidation are keyed by `(scope, deviceId)`; no part of the sync logic or the views refers to a strip by its index in the list (decision D3, addition). `ForEach` uses `StripID` as identity, so a strip keeps its view and its in-progress gesture when neighbors appear or disappear.
- **The displayed list is frozen during a drag.** Stable name order means a hot-plugged device could be inserted *before* the strip being dragged and shift it under the finger. To prevent that, while any drag is in progress the list the view shows doesn't change: strips that arrive are queued and appear when the last drag ends; strips that disappear (other than the one being dragged) stay in place, disabled, until then. `MixerState` itself is updated immediately (the new strip exists and can be read; the gone strip is gone), only the presentation order is deferred. The one exception is the dragged strip's own disappearance, which invalidates the drag right away as above. Criterion in Phase 7: plugging in a device during a drag neither moves nor interrupts the fader being dragged.
...
Mixer in compact width (iPhone): decision §21 D5. Horizontal scrolling with two stacked groups (Output above, Input below), each with its strips in a row, and a page indicator. The iPhone in portrait shows 3–4 strips per group without scrolling.

**Gesture prototype first.** A vertical drag inside a horizontal `ScrollView` is the one interaction in v2 that SwiftUI doesn't give for free: the scroll view and the fader's `DragGesture` compete for the touch, and the result depends on `minimumDistance`, `highPriorityGesture` vs `simultaneousGesture`, `scrollDisabled` while a fader is active and `scrollTargetBehavior(.paging)`. Phase 7 therefore **starts with a throwaway prototype** of just that gesture (a row of dummy vertical faders in a paging horizontal scroll), validated on a physical iPhone against these checks before the real mixer is built: a vertical drag that starts on a fader never scrolls the row; a horizontal swipe that starts on a fader scrolls the row and doesn't change the volume; a drag can start within ~50 ms of touching (no long-press feel); two fingers on two faders work; the page indicator and the scroll position survive a `state` update. If the gesture can't be made reliable, the fallback is **Output/Input tabs** (D5's first alternative), with the strips of one scope filling the screen and no horizontal scrolling; the rest of Phase 7 doesn't change.
...
- **Bluetooth write latency is measured** (decision D8): `setVolume` on AirPods (or other Bluetooth output) that is not the default, at least 50 writes while dragging the menu slider, with median and p95 in the PR; if p95 exceeds 5 ms, moving HAL writes off the main actor (§14.3) is added to Phase 7's scope, otherwise it's closed as not needed;
- paired devices, login item and About live in the Settings window (`⌘,`); the menu keeps the strips, the service status, "Pair New Device…" and "Settings…" (decision D7); manual checklist: opening Settings from the menu brings the window to the front, over whatever app is active, on the first and on repeated opens;
...
**Phase 7 — Protocol v4 and per-strip mixer on the iPhone.** New models in `LevelDeckKit/Protocol` (§15.5), `ProtocolVersion.current = 4`, `AudioServerBridge` emits `strips` and `defaults`, lazy snapshot and single encoding per tick in the server (§16.1). Client: `MixerState` per `StripID`, invalidation on strip disappearance, on-demand per-strip throttle, observable `StripModel` per strip, `HiddenStrips`, per-strip RTT (§16.2). The phase opens with the gesture prototype of §17.2 (vertical faders in a paging horizontal scroll) validated on a physical iPhone; only then is the compact mixer built with two groups and horizontal scrolling (D5), or with Output/Input tabs if the prototype fails. Then: banner with reserved space (A1), fixed-height zones per strip (A3), default mark and action on the strip (A4), lock instead of note (A6), hide and show strips. The device picker and `DevicePickerView` are retired.
*Done when:*
- the gesture prototype passes the §17.2 checks on a physical iPhone, with a short note in the PR (what worked, what was tuned), or the tab fallback was chosen and the reason is recorded;
...
- plugging in or unplugging a device while dragging a fader neither moves nor interrupts the fader being dragged (decision D3, addition): the new strip appears when the drag ends, and the dragged strip's `setVolume` messages keep going to the same `deviceId` (test with the mock and the loopback server: a `state` with an extra strip inserted alphabetically before the dragged one arrives mid-drag, the drag continues and the sends are unchanged);
...
*Done when:* defined when Phase 10 closes. At minimum: one strip per app producing audio, in the menu and in the clients; an app at 100 % and unmuted has no tap or aggregate (G13) and returning to 100 % detaches them; the agent returns to the "no taps" state when it quits and cleans up leftovers on launch; the permission is explained in the menu before it's requested; and the latency measured in the spike holds in the real implementation.
...
The spike (Phase 10) answers each row with a measured value. "Go" requires every mandatory row to be green. The thresholds were fixed in 2.1, before measuring.
...
| G13 | **An app at 100 % and unmuted has no tap attached** (decision D10, addition). Gain 1.0 is not enough: there must be no tap and no aggregate for that app, so its audio never passes through the agent and adds no latency. Returning to 100 % detaches the tap. | Verified two ways: `kAudioHardwarePropertyTapList` (or the aggregate list) shows nothing for that app while at 100 %, and its measured latency equals the no-agent baseline; after moving to 80 % and back to 100 %, both checks pass again within ≤ 1 s | Yes |

"Go with conditions" is the expected outcome if G1–G9 and G13 pass and G11 is uncomfortable: it's implemented with a "Per-App Volume" switch, off by default in the menu, that explains the permission and the indicator before turning on.
...
All ten were **decided in 2.1**: the recommended option in every case, with the additions in the last column. The alternatives are kept for the record. The original list, with more context, is at the top of the planning PR.

| # | Decision | Decided (2.1) | Alternatives and tradeoffs (for the record) | Additions |
|---|---|---|---|---|
...
| D5 | **Compact (iPhone) mixer with many strips** | Two stacked groups (Output above, Input below), vertical strips in horizontal scrolling per group, page indicator. | *Output/Input tabs*: more fader height, but you don't see both sides at once (v1 showed them together). *Vertical list with horizontal sliders*: everything fits without scrolling, but it stops being a mixer and loses v1's vertical gesture. | Phase 7 starts with a gesture prototype (vertical faders in a horizontal scroll) validated on a physical iPhone before the full mixer (§17.2). If the gesture can't be made reliable, the fallback is Output/Input tabs. |
...
| D7 | **Mac menu with many strips** | Collapsible sections with scrolling in the menu (C1), and move paired devices and login item to a Settings window (C4). | *Defaults only in the menu + a "Mixer" window* (C1 alternative, C7): a short menu like today, but the window is one more surface to design and maintain. *Everything in the menu, no separate settings*: zero new windows, but the menu with 12 strips plus paired devices plus login item is uncomfortable. | Manual checklist in Phase 6: the Settings window opens in front when launched from the menu (§14.1). |
| D8 | **HAL writes off the main actor** | Deferred to Phase 6's measurement (§14.3): only if a write exceeds ~5 ms. | *Move now*: avoids the risk up front, but adds an actor and isolation hops to code that's simple and easy to test today. | Measuring write latency on a Bluetooth device is an explicit Phase 6 "Done when" criterion (§19). |
